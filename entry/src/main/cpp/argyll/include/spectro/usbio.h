...
/* HarmonyOS (OHOS) USB DDK context. */
/* OHOS changes: new platform backend using the OHOS USB DDK */
/* (usbio_ohos.c). The device is identified by a DDK deviceId, */
/* and opened by claiming its interface(s). */
struct usb_idevice {
	/* icompath stuff: */
	char *dpath;			/* Device path */
	unsigned int vid, pid;	/* USB Vendor/Product ID (used to re-find the */
							/* device after a DDK session restart) */
	unsigned int iserialno;
	char *SerialNumber;		/* If not-NULL, USB serial number string */
	int nconfig;			/* Number of configurations */
	int config;				/* This config (always 1) */
	...
	usb_ep ep[32];			/* Information about each end point for general usb i/o */
	/* Stuff setup when device is open: */
	unsigned long long deviceId;		/* OHOS USB DDK device id */
	unsigned long long ifaceHandle[32];/* Claimed interface handles, indexed by bInterfaceNumber */
	int nclaimed;			/* Number of claimed interfaces */
};
