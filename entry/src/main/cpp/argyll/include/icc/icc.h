...
#else
			/* OHOS changes: C++ does not allow type declarations inside an */
			/* anonymous union - hoist the struct definition out and use it */
			/* in the union below. (Layout only matters for the C build; */
			/* this type is not instantiated by the i1Pro3 subset.) */
			struct icmMsfResolution {
				unsigned int xres;			/* DPI */
				unsigned int yres;
			};
			union {								/* Setting values - type depends on Sig */
				struct icmMsfResolution *resolution;
				icDevSetMsftMedia   *media;
				icDevSetMsftDither  *halftone;
				unsigned char       *unknown;		/* If an unknown settingSig, count of size, */
													/* access i'th at unknown[ssize * i] */
