...
#if defined(PBRT_WITH_PROGRESS_PNG)
#include <progress_png.h>  // OHOS/Windows port: progress PNG export
#endif
...
void RenderWavefront(BasicScene &scene) {
    ...
    // Render!
#if defined(PBRT_WITH_PROGRESS_PNG)
    StartProgressPNGExport(integrator->film);  // OHOS/Windows port: progress PNG export
#endif
    Float seconds = integrator->Render();
#if defined(PBRT_WITH_PROGRESS_PNG)
    StopProgressPNGExport();  // OHOS/Windows port: progress PNG export
#endif
