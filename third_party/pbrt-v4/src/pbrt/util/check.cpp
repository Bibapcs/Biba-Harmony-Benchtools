...
#ifdef PBRT_IS_LINUX
#include <cxxabi.h>
// musl libc (e.g. OpenHarmony) does not provide <execinfo.h>; only use the
// backtrace()-based stack trace when the header is available. On Android
// (bionic) the header exists but backtrace() is only declared when targeting
// API 33+, so gate on __ANDROID_API__ as well.
#if defined(__has_include)
#if __has_include(<execinfo.h>)
#if !defined(__ANDROID__) || __ANDROID_API__ >= 33
#include <execinfo.h>
#define PBRT_HAVE_EXECINFO_H
#endif
#endif
#endif
#endif
...
void PrintStackTrace() {
    ...
    free(strs);
#elif defined(PBRT_IS_LINUX) && defined(PBRT_HAVE_EXECINFO_H)
    void *callstack[32];
    int frames = backtrace(callstack, PBRT_ARRAYSIZE(callstack));
    char **strs = backtrace_symbols(callstack, frames);
    for (int i = 0; i < frames; ++i) {
...
#else
    // Stack traces are unsupported on this platform (e.g. musl libc without
    // <execinfo.h>); nothing to print.
#endif
}
